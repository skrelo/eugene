const url = "https://premierppm.appfolio.com/listings";
...
class Premier extends Sites {
	constructor() {
		super( url );
		this.name = "Premier Property Management";
		this.source = "premier";
		this.image = "http://www.premierpropertymanagementservices.net/images/logo.gif";
		this.active = true;
	}
		...
		return super.execPage().then( ( $ ) => {
			...
			return Promise.each(listings, (item ) => {
				...
				let listing = {
					...
					link       : 'https://premierppm.appfolio.com' + $( item ).find( '.js-listing-title a' ).attr( 'href' ),
				...
				box.each( ( index,details ) => {
					...
					switch ( label ) {
						...
						case 'Square Feet':
							...
							break;
						case 'Bed / Bath':
							//let bb = value.match( /(\d{1,2}) bd \/ (.*?) ba/ );
							let bb = value.match( /^(.*?) \/ (.*?)$/ );
